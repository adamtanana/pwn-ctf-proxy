...
struct ecdsa {
  ...
  ecdsa(const std::string &public_key, const std::string &private_key,
        const std::string &public_key_password,
        const std::string &private_key_password, const EVP_MD *(*md)(),
        const std::string &name, size_t siglen)
  ...
  void verify(const std::string &data, const std::string &signature) const {
    ...
    if (ECDSA_do_verify((const unsigned char *)hash.data(), hash.size(),
                        sig.get(), pkey.get()) != 1)
      throw signature_verification_exception("Invalid signature");
#endif
  }
...
struct es256 : public ecdsa {
  ...
  explicit es256(const std::string &public_key,
      ...
      : ecdsa(public_key, private_key, public_key_password,
              private_key_password, EVP_sha256, "ES256", 64) {}
};
/**
 * ES384 algorithm
 */
struct es384 : public ecdsa {
  ...
  explicit es384(const std::string &public_key,
                 ...
                 const std::string &private_key_password = "")
      : ecdsa(public_key, private_key, public_key_password,
              private_key_password, EVP_sha384, "ES384", 96) {}
};
/**
 * ES512 algorithm
 */
struct es512 : public ecdsa {
  ...
  explicit es512(const std::string &public_key,
                 ...
                 const std::string &private_key_password = "")
      : ecdsa(public_key, private_key, public_key_password,
              private_key_password, EVP_sha512, "ES512", 132) {}
};
...
public:
  ...
  claim() : val() {}
  JWT_CLAIM_EXPLICIT claim(std::string s) : val(std::move(s)) {}
  ...
  JWT_CLAIM_EXPLICIT claim(const picojson::value &val) : val(val) {}

  template <typename Iterator>
  claim(Iterator start, Iterator end) : val(picojson::array()) {
   ...
   * \return input stream
   */
  inline std::istream &operator>>(std::istream &is) { return is >> val; }
...
public:
   ...
   */
  const std::string &get_header_base64() const noexcept {
   ...
   */
  const std::string &get_payload_base64() const noexcept {
   ...
   */
  const std::string &get_signature_base64() const noexcept {
